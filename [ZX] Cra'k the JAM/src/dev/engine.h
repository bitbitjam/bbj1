...
// Funciones:
unsigned char bs;
MALOTE guarda = {0, 0, 0, 0, 0, 0, 0, 0, 0};
...
void game_ending (void) {
	...
	print_str (1, 3, 122, "THANKS FOR");
	print_str (4, 4, 122, "PLAYING.");
	print_str (1, 6, 32, "BITBITJAM!");
	print_str (5, 7, 32, "CRAKED!!");

	print_str (1, 20, 120, "CODE, DESIGN: JDROMEDARIO");
	print_str (1, 21, 120, "GFX, DESIGN: BROTOLA");
	print_str (1, 22, 120, "MORAL SUPPORT: D-SKYWALK");
	...
	if (!possee && !player.gotten) {
		...
		if (player.vx == 0) {
		    if (attr (gpxx, gpyy) != 16)
			    player.next_frame = player_frames [2];
			else
			    player.next_frame = player_frames [6];
		} else {
			player.subframe ++;
			if (player.subframe == 3) {
				player.subframe = 0;
				player.frame ^= 1;
#ifdef PLAYER_STEP_SOUND
				//step ();
...
void __FASTCALL__ draw_scr_background (void) {
  ...
  //JDRO  Aqui puedo poner la frecuencia con la que se repite el tile 19 (estrellas?) 
		map_attr [gpit] = comportamiento_tiles [gpd];
		if (gpd == 0 && (rand () & 3) == 1) gpd = 19;
		map_buff [gpit] = gpd;
#endif	
	...
	for (gpit = 0; gpit < 3; gpit ++) {
		...
		switch (gpt) {
			...
			case 3:
			    ...
			    // guarda, misma posicion del perrete
			    switch(guarda.t)
			    {
                    case 31: //derecha
                        if(gpt != 2)
                        ...
                        if(guarda.my)
                        {
                            if( ((gpy < 100) && (guarda.x <= gpx)) || (guarda.x > gpx))
                            {
                                guarda.t = 10;
                                draw_coloured_tile (VIEWPORT_X + (guarda.x >> 3) , VIEWPORT_Y + (guarda.y >> 3) , 10); 
                                draw_coloured_tile (VIEWPORT_X + (guarda.x >> 3), VIEWPORT_Y + (guarda.y >> 3) + 2, 44);
                            } 
                            guarda.my = 0;
                            //peta_el_beeper(9);
                        }
                        if(((gpy < 100) || (guarda.x >= gpx)) && (en_an [gpit].state == TYPE_6_IDLE))
                            continue;
                        break;
                    case 10: // izq
                        if(gpt != 3)
                        ...
                        if(guarda.my)
                        {
                            ...
                            {
                                guarda.t = 31;
                                draw_coloured_tile (VIEWPORT_X + (guarda.x >> 3), VIEWPORT_Y + (guarda.y >> 3), 31); 
                                draw_coloured_tile (VIEWPORT_X + (guarda.x >> 3), VIEWPORT_Y + (guarda.y >> 3) + 2, 34);
                            }
                            guarda.my = 0;
                            //peta_el_beeper(9);
                        }
				...
				malotes [enoffsmasi].x += malotes [enoffsmasi].mx << 3;
