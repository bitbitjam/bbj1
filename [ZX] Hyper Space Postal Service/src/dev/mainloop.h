	...
	while (1) {
		...
		while (playing) {
			...
			if (player.life != life_old) {
				...
				//player.life=15;  //VIDAS INFINITAS
				print_str (2, 21, COLOR_BLANCO_FOREGROUND+COLOR_LIGHT, "LIFE: ###############  ");
				life_old=15;
				while(player.life<life_old)
				{
					print_str (7+life_old, 21, COLOR_BLANCO_FOREGROUND+COLOR_LIGHT, " ");
					life_old=life_old-1;
				}
